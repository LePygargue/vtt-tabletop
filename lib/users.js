...
/** Variante asynchrone (pool de threads) : une connexion ne bloque pas les autres joueurs. */
const scryptAsync = promisify(crypto.scrypt);
async function scryptHashAsync(password, salt) {
  return (await scryptAsync(String(password), salt, 64)).toString('hex');
}
...
function createUser({ username, password, displayName, color }) {
  ...
  byUsername.set(user.username, user.id);
  saveUsers();
  return user;
}
...
async function verifyPassword(user, password) {
  const hash = await scryptHashAsync(password, user ? user.salt : DUMMY_SALT);
