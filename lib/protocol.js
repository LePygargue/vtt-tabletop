...
const {
  rooms, cellKey, remapFog, touch, scheduleSave, snapCoord, pub, publicTokens, fogSnapshot, pubStroke, pubImage, pubJournalEntry,
...
function handleJoin(client, msg) {
  ...
  const keyOk = !!msg.gmKey && safeEqual(msg.gmKey, room.gmKey);
  if (keyOk || room.gmUserIds.has(client.user.id)) {
    client.role = 'gm';
    if (!room.gmUserIds.has(client.user.id)) {
      // La clé MJ rattache durablement le salon à ce compte (retrouvé depuis n'importe quel navigateur).
