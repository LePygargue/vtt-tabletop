...
const { onlineTokenIds } = require('./net');
...
/** Crée un salon ; son créateur (compte) en devient MJ, sur tous ses appareils. */
function createRoom(ownerUserId) {
  const room = {
    id: randomId(8),
    gmKey: randomHex(24),
    ...
    bannedPlayers: new Map(), // playerId -> nom au moment de l'exclusion (empêche de rejoindre ce salon)
    ...
    gmUserIds: new Set(ownerUserId ? [ownerUserId] : []), // comptes MJ du salon : créateur + comptes ayant prouvé la gmKey
  };
...
function serializeRoom(r) {
  return {
    ...
    bannedPlayers: bannedList(r),
    initiative: r.initiative,
    danger: r.danger,
    desperation: r.desperation,
    gmUserIds: [...r.gmUserIds],
  };
}
...
function buildRoom(s, now) {
  const room = {
    ...
    bannedPlayers: new Map((s.bannedPlayers || []).map((b) => [b.playerId, b.name])),
    ...
    clients: new Set(),
    rolls: [],
    gmUserIds: new Set((Array.isArray(s.gmUserIds) ? s.gmUserIds : []).filter((id) => typeof id === 'string')),
  };
  if (s.workplace) {
...
function pub(t) {
  const out = { id: t.id, name: t.name, color: t.color, size: t.size, x: t.x, y: t.y, hidden: !!t.hidden, pc: !!t.owner, conditions: t.conditions || [] };
  // Présentation d'un PNJ (portrait + description) : publique, elle suit la visibilité du jeton
  if (!t.owner) {
    out.portrait = t.portrait || null;
    out.description = t.description || '';
  }
  return out;
}
function publicTokens(room, client) {
  return [...room.tokens.values()]
    .filter((t) => client.role === 'gm' || !t.hidden || (t.visibleTo && t.visibleTo.includes(client.playerId)))
    .map(pub);
}
...
/** Liste des joueurs exclus (MJ uniquement). */
function bannedList(room) {
  return [...room.bannedPlayers.entries()].map(([playerId, name]) => ({ playerId, name }));
}
function stateFor(room, client) {
  ...
  return {
    ...
    strokes: [...w.strokes.values()].filter((s) => isGM || s.layer !== 'gm').map(pubStroke),
    images: [...room.images.values()].filter((i) => isGM || !i.hidden).sort((a, b) => a.order - b.order).map(pubImage),
    tokens: publicTokens(room, client),
    ...
    online: onlineTokenIds(room),
    rolls: room.rolls.filter((r) => rollVisible(r, client)),
    bannedPlayers: isGM ? bannedList(room) : undefined,
    gmKey: isGM ? room.gmKey : undefined, // le MJ reconnu par son compte récupère la clé (lien MJ, en-têtes HTTP)
  };
}
...
module.exports = {
  ...
  snapCoord, pub, publicTokens, fogSnapshot, pubStroke, pubImage, pubJournalEntry, pubInitiative, bannedList, stateFor, newPlayerToken,
};
