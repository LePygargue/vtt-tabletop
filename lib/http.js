...
const { MAX_UPLOAD, MAX_ROOMS, MAX_IMAGES, UPLOAD_DIR, PUBLIC_DIR, MIME, SECURITY_HEADERS } = require('./config');
const { randomHex, safeEqual, cleanName } = require('./util');
const { rooms, createRoom, scheduleSave, deleteRoom } = require('./rooms');
const { addImage, removeImageFile } = require('./protocol');
...
function allowCreate(ip) {
  const now = Date.now();
  const arr = (createLog.get(ip) || []).filter((t) => now - t < 3600e3);
...
async function handleRequest(req, res) {
  ...
  if (req.method === 'POST' && p === '/api/login') {
    ...
    if (!body || !verifyPassword(target, body.password)) return json(res, 401, { error: 'Identifiants invalides.' });
  ...
  if (req.method === 'POST' && m) {
    ...
    fs.writeFileSync(path.join(UPLOAD_DIR, file), body);
  ...
  if (p.startsWith('/uploads/')) {
    if (!user) { res.writeHead(404, SECURITY_HEADERS); return res.end(); }
    const name = path.basename(p);
    if (!/^[A-Z0-9]{8}-[0-9a-f]{12}\.(png|jpg|gif|webp)$/.test(name)) {
      res.writeHead(404, SECURITY_HEADERS);
      return res.end();
    }
    return serveFile(res, path.join(UPLOAD_DIR, name), { 'Cache-Control': 'public, max-age=2592000, immutable' });
  }
