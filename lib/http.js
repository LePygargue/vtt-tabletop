...
/** MJ du salon : compte rattaché au salon, ou clé MJ fournie en en-tête. */
function isRoomGM(req, room, user) {
  return room.gmUserIds.has(user.id) || safeEqual(req.headers['x-gm-key'] || '', room.gmKey);
}
...
async function handleRequest(req, res) {
  ...
  // Salons dont le compte connecté est MJ (liste de l'accueil, indépendante du navigateur)
  if (req.method === 'GET' && p === '/api/rooms/mine') {
    const mine = [...rooms.values()]
      .filter((r) => r.gmUserIds.has(user.id))
      .sort((a, b) => b.lastActive - a.lastActive)
      .map((r) => ({ id: r.id, name: r.name || null }));
    return json(res, 200, { rooms: mine });
  }
  ...
  if (req.method === 'DELETE' && m) {
    ...
    if (!isRoomGM(req, room, user)) return json(res, 403, { error: 'Réservé au MJ.' });
