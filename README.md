...
Le brouillard est visuel : l'image de la carte complète est envoyée aux joueurs, un joueur technique pourrait donc la retrouver. Pour un PNJ vraiment secret, utilise « Caché aux joueurs » (le serveur ne l'envoie pas). Les salons n'ont pas de mot de passe : le code à 8 caractères fait office de secret. Le calque de dessin partagé n'est pas masqué par le brouillard de guerre : un trait posé par le MJ sur une zone non révélée reste visible des joueurs — utilise le calque « MJ caché » pour annoter une zone encore secrète. Un salon créé avant la mise en place des comptes garde ses anciens joueurs anonymes : ils n'ont plus de compte associé, donc ne peuvent plus rejoindre ce salon avec leur ancien jeton (un nouveau compte y crée un jeton neuf). La fiche de personnage ne calcule pas Santé/Volonté automatiquement (ce sont des champs libres) et n'a pas de champ dédié aux mécaniques propres à un type de personnage (Puissance du Sang, Intégrité...) — mets-les dans « Notes ». Chaque carte ajoutée à l'espace de travail reste sur le disque tant que le MJ ne la supprime pas explicitement (jusqu'à 20 par salon) ; pense à retirer les cartes qui ne servent plus.
